...
def validate_recipe_filters(
    ...
    dietary_restriction_normalized = _normalize_filter_value(
        dietary_restriction
    )
    ...
    meat_terms = (
        protein_aliases["carne_bovina"]
        + protein_aliases["frango"]
        + protein_aliases["peixe"]
        + [
            "bacon",
            "presunto",
            "linguiça",
    ...
    animal_terms = meat_terms + protein_aliases["ovo"] + [
        "leite",
        "queijo",
        "manteiga",
        "mel",
    ...
    if dietary_restriction_normalized in {
        "vegetariano",
        "vegetarian"
    }:
    ...
    if protein_type_normalized in {"carne_bovina", "bovina", "beef"}:
        if not any(term in text for term in beef_terms):
            raise HTTPException(
                status_code=400,
                detail="A IA não respeitou o filtro de proteína: carne bovina.",
            )

    if protein_type_normalized in {"frango", "chicken"}:
        if not any(term in text for term in chicken_terms):
            raise HTTPException(
                status_code=400,
                detail="A IA não respeitou o filtro de proteína: frango.",
            )

    if protein_type_normalized in {"peixe", "fish"}:
        if not any(term in text for term in fish_terms):
            raise HTTPException(
                status_code=400,
                detail="A IA não respeitou o filtro de proteína: peixe.",
            )

    if protein_type_normalized in {"ovo", "ovos", "egg", "eggs"}:
        if not any(term in text for term in egg_terms):
            raise HTTPException(
                status_code=400,
                detail="A IA não respeitou o filtro de proteína: ovo.",
            )

    if protein_type_normalized in {"vegetal", "proteina_vegetal", "plant_based"}:
        if not any(term in text for term in plant_protein_terms):
            raise HTTPException(
                status_code=400,
                detail="A IA não respeitou o filtro de proteína: vegetal.",
            )
